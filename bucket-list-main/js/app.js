...
class BucketListApp {
    ...
    createBucketItemHTML(item) {
        ...
        return `
            <div class="bucket-item flex items-center gap-4">
                <!-- 체크박스 -->
                <button
                    class="flex-shrink-0 w-7 h-7 rounded-lg border-2 flex items-center justify-center ${checkboxClass} transition-all cursor-pointer"
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-base font-medium ${completedClass} break-words">${this.escapeHtml(item.title)}</p>
                ...
                <div class="flex gap-2 flex-shrink-0">
                    <button
                        class="px-3.5 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-all text-sm font-semibold"
                        onclick="app.openEditModal('${item.id}', '${this.escapeHtml(item.title).replace(/'/g, "\\'")}')"
                    >
                        수정
                    </button>
                    <button
                        class="px-3.5 py-1.5 bg-rose-50 text-rose-500 rounded-lg hover:bg-rose-100 transition-all text-sm font-semibold"
